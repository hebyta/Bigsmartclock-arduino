...
void calculateClock() {
  ...
  if (clockSeconds >= 60) {
    clockMillis += TIME_OFFSET_MODULE;
    drawMinutes = true;
    clockMinutes++;
    clockSeconds = 0;
  }
...
void calculateChrono() {
  if (isChronoRunning) {
    ...
    if (chronoSeconds >= 60) {
      chronoMillis += TIME_OFFSET_MODULE;
      drawMinutes = true;
      chronoMinutes++;
      chronoSeconds = 0;
    }
...
void calculateCowntdown() {
  if (isCountDownRunning) {
    ...
    if (countDownSeconds < 0 && (countDownMinutes > 0 || countDownHours > 0)) {
      countDownMillis += TIME_OFFSET_MODULE;
      countDownSeconds = 59;
      countDownMinutes--;
      drawMinutes = true;
    }
...
void calculateAutoChrono() {
  if (isAutoChronoRunning) {
    ...
    if (autoChronoSeconds >= 60) {
      drawMinutes = true;
      autoChronoMinutes += TIME_OFFSET_MODULE;
      autoChronoSeconds = 0;
    }
    if (autoChronoMinutes >= 60) {
      drawHours = true;
      autoChronoHours += TIME_OFFSET_MODULE;
      autoChronoMinutes = 0;
    }
...
void calculateAutoCowntdown() {
  if (isAutoCountDownRunning) {
    ...
    if (autoCountDownSeconds < 0 && (autoCountDownMinutes > 0 || autoCountDownHours > 0)) {
      autoCountDownMillis += TIME_OFFSET_MODULE;
